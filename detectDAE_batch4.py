...
class Dinonet:
    ...
    def gakushuu(self,dalo_train,dalo_test,EPOCH,cnt_multi=10):
        ...
        for kaime in range(self.mounankai,self.mounankai+EPOCH):
            ...
            for i_batch,(x,y) in enumerate(dalo_train):
                ...
                if((i_batch+1)%int(np.ceil(dalo_train.nkai/cnt_multi))==0 or i_batch==dalo_train.nkai-1):
                    ...
                    for x,y in test_data:
                        ...
                        plt.tight_layout()
                        plt.savefig(os.path.join(self.save_folder,'denoised_image_DAE_4.png'))
                        plt.close()
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(self.save_folder,'graph_DAE_4.png'))
            plt.close()

            # PSNRの変化を表すグラフを書く
            plt.figure(figsize=[5,4])
            plt.xlabel('trial')
            plt.ylabel('PSNR(DAE: batch size 4) [dB]')
            ar = np.arange(1,kaime+2)
            plt.plot(ar,self.psnr,'#11aa99')
            plt.tight_layout()
            plt.savefig(os.path.join(self.save_folder,'psnr_DAE_4.png'))
            plt.close()
