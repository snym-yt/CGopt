...
class Dinonet:
    ...
    def gakushuu(self,dalo_train,dalo_test,n_loop,n_kaku=5,cnt_multi=10):
        ...
        for kaime in range(self.mounankai,self.mounankai+n_loop):
            ...
            for i_batch,(x,y) in enumerate(dalo_train):
                ...
                if((i_batch+1)%int(np.ceil(dalo_train.nkai/cnt_multi))==0 or i_batch==dalo_train.nkai-1):
                    ...
                    for x,y in test_data:
                        ...
                        plt.tight_layout()
                        plt.savefig(os.path.join(self.save_folder,'denoised_image_Unet_epoch90.png'))
                        plt.close()
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(self.save_folder,'loss_Unet_epoch90.png'))
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(self.save_folder,'psnr_Unet_epoch90.png'))
            plt.close()
